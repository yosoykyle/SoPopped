      ...
      if ($.validator) {
        ...
        window.sopoppedValidateEmailFormat = function (value) {
          try {
            if (
              ...
              typeof window.sopoppedValidate.emailFormat === "function"
            ) {
        ...
        window.sopoppedValidatePasswordStrength = function (value) {
          try {
            if (
              window.sopoppedValidate &&
              typeof window.sopoppedValidate.passwordStrength === "function"
            ) {
        ...
        $.validator.addMethod(
          ...
          function (value, element) {
            ...
            return window.sopoppedValidateEmailFormat
              ? window.sopoppedValidateEmailFormat(value)
              : false;
          },
          function (value, element) {
            if (!value.includes("@")) {
        ...
        $.validator.addMethod(
          ...
          function (value, element) {
            if (this.optional(element)) return true;
            return window.sopoppedValidatePasswordStrength
              ? window.sopoppedValidatePasswordStrength(value)
              : false;
          },
          ...
          $("#loginForm").validate({
            ...
            submitHandler: function (form) {
              ...
              function _itemId(it) {
                if (!it) return null;
                return (
                  it.id || it.product_id || it.productId || it.productId || null
                );
              }
              ...
              async function _saveServerCartArray(arr) {
                ...
                const j = await window.sopoppedFetch
              ...
              function _mergeCartsSumming(localArr, serverArr) {
                ...
                (serverArr || []).forEach((it) => {
                  ...
                  map.set(
                    id,
                    Object.assign({}, it, { id: Number(id), quantity: qty })
                  );
                });
                (localArr || []).forEach((it) => {
                  ...
                  if (map.has(id)) {
                    ...
                    existing.quantity =
                      (Number(existing.quantity || 0) || 0) + qty;
                    map.set(id, existing);
                  } else {
              ...
              async function mergeCartAfterLoginAndUpdateUI() {
                try {
                  ...
                  if (
                    !saveResp ||
                    (typeof saveResp.success !== "undefined" &&
                      !saveResp.success)
                  ) {
                    // treat as failure
